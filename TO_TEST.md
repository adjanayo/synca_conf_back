...
- ⚠️ CI cassée après le push (`image-scan` / Trivy) par une CVE **sans rapport avec ce changement** : `libcrypto3`/`libssl3` (OpenSSL) 3.5.7-r0 → 3.5.8-r0 (CVE-2026-14456, DoS QUIC), publiée après le dernier build de l'image `python:3.12-alpine`. Comme ce tag est une étiquette flottante, un rebuild peut réintroduire des paquets Alpine non patchés même sans changement de code. Fixé en ajoutant `apk upgrade --no-cache` en tête des stages `runtime` et `dev` du `Dockerfile` — vérifié via `apk list -I libcrypto3 libssl3` (3.5.8-r0 confirmé), image toujours à **157 Mo**. Rappel `current-versions-only` : à re-vérifier à chaque build, pas seulement à l'ajout d'une dépendance.
