...
## Corpus Check
- 165 files · ~160,168 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 831 nodes · 1408 edges · 63 communities detected
- Extraction: 69% EXTRACTED · 31% INFERRED · 0% AMBIGUOUS · INFERRED: 430 edges (avg confidence: 0.76)
...
Cohesion: 0.03
Nodes (73): AmbassadorAdminCreate, AmbassadorStatusUpdate, ExhibitorAdminCreate, ExhibitorStatusUpdate, PartnerAdminCreate, PartnerStatusUpdate, SpeakerAdminCreate, SpeakerStatusUpdate (+65 more)
...
Cohesion: 0.04
Nodes (40): create_day(), create_session(), AuditLog, Base, CampaignWindow, ContactMessage, Faq, Base (+32 more)
...
Cohesion: 0.07
Nodes (32): ParticipantTokenOut, main(), Send a transactional email, or log it in dev.      Without RESEND_API_KEY config, send_email(), _generate_code(), InvalidOtpError, Create + email a login code if the address matches a verified account.      Sile, Consume the most recent unexpired, unconsumed code for this email.      Every fa (+24 more)
...
Cohesion: 0.11
Nodes (27): application_received_email(), otp_login_email(), TODO.md: HTML email templates.  Table-based, inline-CSS layout (the only markup, registration_confirmed_email(), _render(), ticket_delivered_email(), apply_as_ambassador(), apply_as_exhibitor() (+19 more)
...
Cohesion: 0.16
Nodes (13): get_current_admin(), require_permission(), make_admin(), test_any_authenticated_admin_can_read_contact_messages(), test_login_success_grants_access_to_a_permitted_view(), test_login_wrong_password_is_rejected(), test_role_without_permission_gets_403_on_gated_view(), make_admin_with_role() (+5 more)
...
## Knowledge Gaps
- **33 isolated node(s):** `Validate a multipart form's non-file fields against a Pydantic model.      Works`, `Application-layer encryption for genuinely sensitive PII (7.8).      Not for fie`, `Verify admin credentials, enforcing the account-lockout policy.      Always take`, `Issued after a successful OTP verify (app/api/participant_auth.py).      Distinc`, `Generate the ticket's PDF+QR, upload it, and email it.      Runs as a Background` (+28 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **Thin community `Community 49`** (2 nodes): `list_audit_logs()`, `admin_audit.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 50`** (2 nodes): `list_waitlist()`, `admin_waitlist.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 51`** (2 nodes): `test_health()`, `test_health.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 61`** (1 nodes): `__init__.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 62`** (1 nodes): `Generate the ticket's PDF+QR, upload it, and email it.      Runs as a Background`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.