...
## Corpus Check
- 172 files · ~167,562 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 873 nodes · 1471 edges · 68 communities detected
- Extraction: 70% EXTRACTED · 30% INFERRED · 0% AMBIGUOUS · INFERRED: 445 edges (avg confidence: 0.76)
...
8. `make_admin_with_permission()` - 18 edges
9. `get_settings()` - 17 edges
10. `Payment` - 16 edges
...
- `Rappels récurrents waitlist (voir DEVLOG.md front, phase J3 suite).  Pas de cron` --uses--> `CampaignWindow`  [INFERRED]
  /Users/kodjododjango/Downloads/dev_projects/synca_conf_back/app/services/waitlist_reminder.py → /Users/kodjododjango/Downloads/dev_projects/synca_conf_back/app/models/campaign.py
...
Cohesion: 0.03
Nodes (80): AmbassadorAdminCreate, AmbassadorStatusUpdate, ExhibitorAdminCreate, ExhibitorStatusUpdate, PartnerAdminCreate, PartnerStatusUpdate, SpeakerAdminCreate, SpeakerStatusUpdate (+72 more)

### Community 1 - "Community 1"

Cohesion: 0.04
Nodes (51): AuditLog, Base, CampaignWindow, ContactMessage, Faq, Base, DeclarativeBase, Run migrations in 'offline' mode.      This configures the context with just a U (+43 more)

### Community 2 - "Community 2"

Cohesion: 0.05
Nodes (48): create_pass_type(), create_promo_code(), Payment, PromoCode, Ticket, Waitlist, PassType, make_admin_with_permission() (+40 more)
...
Cohesion: 0.06
Nodes (34): create_ambassador_admin(), create_exhibitor_admin(), create_partner_admin(), create_speaker_admin(), update_ambassador_status(), Ambassador, Exhibitor, Partner (+26 more)
...
Cohesion: 0.09
Nodes (33): application_received_email(), otp_login_email(), TODO.md: HTML email templates.  Table-based, inline-CSS layout (the only markup, registration_confirmed_email(), _render(), ticket_delivered_email(), waitlist_reminder_email(), waitlist_ticketing_open_email() (+25 more)
...
Cohesion: 0.09
Nodes (17): _is_open(), _notify_waitlist(), update_campaign_window(), BaseHTTPMiddleware, Send a transactional email, or log it in dev.      Without RESEND_API_KEY config, send_email(), lifespan(), Pas de cron dans le projet : boucle asyncio en tâche de fond,     voir app/servi (+9 more)
...
Cohesion: 0.12
Nodes (19): register(), make_user(), test_null_special_needs_stays_null(), test_orm_read_returns_plaintext(), test_raw_db_row_is_not_plaintext(), make_verified_user(), test_request_otp_only_creates_row_for_known_verified_email(), test_request_otp_rate_limited_after_three_per_15_minutes() (+11 more)
...
Cohesion: 0.09
Nodes (12): create_day(), create_session(), Day, Session, test_list_days_ordered(), test_sessions_filter_by_day_and_category_excludes_private(), test_day_unique_date(), test_partner_level_and_faq_category() (+4 more)
...
## Knowledge Gaps
- **38 isolated node(s):** `Validate a multipart form's non-file fields against a Pydantic model.      Works`, `Application-layer encryption for genuinely sensitive PII (7.8).      Not for fie`, `Verify admin credentials, enforcing the account-lockout policy.      Always take`, `Issued after a successful OTP verify (app/api/participant_auth.py).      Distinc`, `Generate the ticket's PDF+QR, upload it, and email it.      Runs as a Background` (+33 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **Thin community `Community 52`** (2 nodes): `list_audit_logs()`, `admin_audit.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 53`** (2 nodes): `list_waitlist()`, `admin_waitlist.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 54`** (2 nodes): `test_health()`, `test_health.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 55`** (1 nodes): `__init__.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 56`** (1 nodes): `__init__.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 57`** (1 nodes): `rate_limit.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 64`** (1 nodes): `__init__.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 65`** (1 nodes): `Verify admin credentials, enforcing the account-lockout policy.      Always take`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 66`** (1 nodes): `Issued after a successful OTP verify (app/api/participant_auth.py).      Distinc`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 67`** (1 nodes): `Generate the ticket's PDF+QR, upload it, and email it.      Runs as a Background`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.