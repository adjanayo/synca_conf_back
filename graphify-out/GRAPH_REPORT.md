# Graph Report - /Users/kodjododjango/Downloads/dev_projects/synca_conf_back  (2026-09-16)

## Corpus Check
- 188 files · ~203,449 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1038 nodes · 1797 edges · 84 communities detected
- Extraction: 69% EXTRACTED · 31% INFERRED · 0% AMBIGUOUS · INFERRED: 565 edges (avg confidence: 0.75)
...
3. `Base` - 38 edges
4. `PassType` - 30 edges
5. `upload_file()` - 23 edges
6. `User` - 20 edges
7. `Role` - 20 edges
8. `AdminUser` - 19 edges
9. `make_admin_with_permission()` - 19 edges
10. `get_settings()` - 18 edges
...
Cohesion: 0.02
Nodes (100): AmbassadorAdminCreate, AmbassadorStatusUpdate, ExhibitorAdminCreate, ExhibitorStatusUpdate, PartnerAdminCreate, PartnerStatusUpdate, SpeakerAdminCreate, SpeakerStatusUpdate (+92 more)

### Community 1 - "Community 1"

Cohesion: 0.04
Nodes (58): create_ambassador_admin(), create_exhibitor_admin(), create_partner_admin(), create_speaker_admin(), update_ambassador_status(), update_exhibitor_status(), update_partner_status(), update_speaker_status() (+50 more)

### Community 2 - "Community 2"

Cohesion: 0.06
Nodes (62): create_access_token(), AdminUser, Permission, Role, RolePermission, make_admin_with_permission(), make_ambassador(), make_exhibitor() (+54 more)

### Community 3 - "Community 3"

Cohesion: 0.04
Nodes (45): create_partner_benefit(), create_day(), create_session(), update_day(), update_session(), AuditLog, Base, CampaignWindow (+37 more)
...
Cohesion: 0.04
Nodes (48): _aware(), _is_open(), _notify_waitlist(), update_campaign_window(), ParticipantTokenOut, BaseHTTPMiddleware, Send a transactional email, or log it in dev.      Without RESEND_API_KEY config, send_email() (+40 more)

### Community 6 - "Community 6"

Cohesion: 0.07
Nodes (46): join_waitlist(), Payment, PromoCode, Ticket, Waitlist, PassType, open_ticketing(), register_payload() (+38 more)

### Community 7 - "Community 7"

Cohesion: 0.08
Nodes (37): create_team(), create_team_member(), delete_team(), _get_team_or_404(), update_team(), update_team_member(), HackathonTeam, HackathonTeamMember (+29 more)

### Community 8 - "Community 8"

Cohesion: 0.09
Nodes (25): make_user(), test_null_special_needs_stays_null(), test_orm_read_returns_plaintext(), test_raw_db_row_is_not_plaintext(), make_verified_user(), test_request_otp_only_creates_row_for_known_verified_email(), test_request_otp_rate_limited_after_three_per_15_minutes(), test_request_otp_same_generic_response_known_and_unknown_email() (+17 more)

### Community 9 - "Community 9"

Cohesion: 0.08
Nodes (20): downgrade(), encrypt users phone_whatsapp and special_needs (PII 7.8)  Revision ID: d7d5f8910, upgrade(), BaseSettings, get_settings(), Settings, db_session(), EncryptedString (+12 more)

### Community 10 - "Community 10"

Cohesion: 0.13
Nodes (18): create_admin_user(), list_admin_users(), _to_read(), update_admin_user(), main(), hash_password(), validate_password_strength(), verify_password() (+10 more)

### Community 11 - "Community 11"

Cohesion: 0.13
Nodes (17): create_partner_level(), delete_partner_level(), _get_level_or_404(), _resolve_benefits(), update_partner_benefit(), update_partner_level(), PartnerLevel, form_fields() (+9 more)
...
Cohesion: 1.0
Nodes (1): Raised when the object storage (B2) call itself fails -- misconfiguration     or

### Community 74 - "Community 74"

Cohesion: 1.0
Nodes (1): Reject anything whose bytes aren't actually a decodable image.      A client-sup

### Community 75 - "Community 75"

Cohesion: 1.0
Nodes (1): Downscale + re-encode an image before it leaves for object storage.      Keeps t
...
## Knowledge Gaps
- **69 isolated node(s):** `Validate a multipart form's non-file fields against a Pydantic model.      Works`, `Application-layer encryption for genuinely sensitive PII (7.8).      Not for fie`, `Sous-ensemble sans PII de SpeakerRead, pour les endpoints publics (liste + détai`, `Sous-ensemble sans PII de AmbassadorRead, pour les endpoints publics (liste + dé`, `Sous-ensemble sans PII de PartnerRead, pour l'endpoint public /api/partners.` (+64 more)
...
- **Thin community `Community 72`** (1 nodes): `__init__.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 73`** (1 nodes): `Raised when the object storage (B2) call itself fails -- misconfiguration     or`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 74`** (1 nodes): `Reject anything whose bytes aren't actually a decodable image.      A client-sup`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 75`** (1 nodes): `Downscale + re-encode an image before it leaves for object storage.      Keeps t`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 76`** (1 nodes): `Reject anything whose bytes aren't actually a decodable image.      A client-sup`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 77`** (1 nodes): `Sous-ensemble minimal de `User` pour la recherche/liaison admin (ex.     membres`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 78`** (1 nodes): `Création directe d'un compte participant par un admin (pas via     l'inscription`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 79`** (1 nodes): `Generate the ticket's PDF+QR, upload it, and email it.      Runs as a Background`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 80`** (1 nodes): `Validate a multipart form's non-file fields against a Pydantic model.      Works`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 81`** (1 nodes): `Verify admin credentials, enforcing the account-lockout policy.      Always take`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 82`** (1 nodes): `Issued after a successful OTP verify (app/api/participant_auth.py).      Distinc`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 83`** (1 nodes): `Generate the ticket's PDF+QR, upload it, and email it.      Runs as a Background`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.