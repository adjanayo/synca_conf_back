...
| 4.3 | Candidature speaker | `POST /api/speakers/apply` | gardé par fenêtre `call_for_speaker` (4.11), upload photo (MIME réel + `Pillow` pour vérifier que c'est une vraie image), `status=pending` | ✅ Test Done |
| 4.4 | Candidature ambassadeur | `POST /api/ambassadors/apply` | gardé par fenêtre `call_for_ambassador` (4.11), si accepté plus tard → génération `promo_code` unique (Phase 6) | ✅ Test Done |
| 4.5 | Candidature partenaire | `POST /api/partners/apply` | gardé par fenêtre `call_for_partner` (4.11), upload logo, workflow `pending→contacted→negotiating→confirmed|rejected` | ✅ Test Done |
| 4.6 | Candidature exposant | `POST /api/exhibitors/apply` | gardé par fenêtre `call_for_exhibitor` (4.11), `visuals_url` = lien externe (Drive/WeTransfer) par `schema.md` §3F, pas un upload fichier, mêmes workflow/statuts que partenaires | ✅ Test Done |
...
| 4.12 | Emails transactionnels (Resend) | accusé réception, confirmation inscription | prod : envoi réel Resend ; dev : backend console `loguru` (email loggé, pas envoyé) — pas de conteneur SMTP de test | ✅ Test Done — image 198 Mo en CI, ramenée à **177 Mo** par un strip des symboles de debug des extensions C (voir note juste après ce tableau) |
...
| 7.3 | Rate limiting global + par endpoint sensible | `slowapi` : 60/min public, 30/min admin, 5/min login, 3/min formulaires publics | ✅ Test Done — amendement : `default_limits` de `slowapi` (mode middleware) ne matche pas les routes montées via `include_router` dans cette stack FastAPI/Starlette (`_find_route_handler` retourne toujours `None`), donc chaque route porte son propre `@limiter.limit(...)` explicite plutôt que de compter sur un défaut global implicite |
